...
Vec3f eye(6,0,10); // x y distance : position oeil
Vec3f center(0,0,0); // x y z : coordonnée focus de l oeil
...
void triangleBuffIntense(Triangle tri,  float *buffer,TGAImage &image,float intensity,Vec2f *texture){
/*
...
*/
    ...
    if (intensity>.85) intensity = 1;
				...
				if (buffer[int(xx+yy*width)]<=z) {
					...
					TGAColor couleur = TGAColor(0*intensity, 255*intensity, 0*intensity, 255);
					buffer[int(xx+yy*width)] = z;
					image.set(xx,yy, color);
            	}
			}
	...
	for (int i=0; i<model->nfaces(); i++) {
    	...
    	float intensity = n*light_dir;
   		...
   		if (intensity>0) {
			...
			triangleBuffIntense(*tri,buffer, image, intensity,text);
    	}
	}
